...
- Server uses Yogi optimizer for aggregation
- Yogi is a variant of Adam that uses additive instead of multiplicative second moment
- More stable than Adam, especially for non-IID federated settings
...
v_t = v_{t-1} - (1 - β2) * Δ_t^2 * sign(v_{t-1} - Δ_t^2)  # Yogi update (key difference from Adam)
m_hat = m_t / (1 - β1^t)
v_hat = v_t / (1 - β2^t)
...
class FedYogiServer(BaseServer):
    ...
    def train(self):
        """Train using FedYogi with server-side Yogi optimization"""
        ...
        for i in trange(self.num_rounds, desc=self.desc):
            ...
            # Aggregate using Yogi-style adaptive optimization
    ...
    def aggregate_yogi(self, wsolns):
        """
        Aggregate client updates using Yogi optimizer
        
        Key difference from Adam: Uses additive update for second moment
        v_t = v_{t-1} - (1 - β2) * Δ_t^2 * sign(v_{t-1} - Δ_t^2)
        Instead of Adam's multiplicative:
        v_t = β2 * v_{t-1} + (1 - β2) * Δ_t^2
        """
        self.t += 1
        total_weight = 0.0
        ...
        for key in current_state.keys():
            ...
            # Second moment (Yogi-style additive update - KEY DIFFERENCE from Adam)
            grad_squared = grad ** 2
            self.v[key] = self.v[key] - (1 - self.beta2) * grad_squared * torch.sign(self.v[key] - grad_squared)
            ...
            new_state[key] = current_state[key] + self.server_lr * m_hat / (torch.sqrt(torch.abs(v_hat)) + self.tau)
