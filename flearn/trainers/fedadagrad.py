"""
FedAdagrad: Adaptive Federated Optimization with Adagrad
Paper: "Adaptive Federated Optimization" (Reddi et al., MLSys 2021)
https://arxiv.org/abs/2003.00295

Key Idea:
- Clients perform standard SGD training
- Server uses Adagrad-style adaptive learning rates for aggregation
- Accumulates squared gradients for adaptive step sizes
- Particularly effective for non-IID data with varying gradient scales

Algorithm:
Server maintains:
- v_t: accumulated squared pseudo-gradients
- Server learning rate η
- tau: adaptive learning rate parameter (typically 1e-3 to 1e-2)

Update rule:
Δ_t = Σ_i (p_i * Δx_i)  # weighted average of client updates
v_t = v_t + Δ_t^2        # accumulate squared gradients
x_t+1 = x_t + η * Δ_t / (sqrt(v_t) + τ)  # adaptive update
"""
...
class FedAdagradServer(BaseServer):
    def __init__(self, params):
        print("Using FedAdagrad (Adaptive Federated Optimization with Adagrad) to Train")
        
        # FedAdagrad hyperparameters
        ...
        self.v = None  # Accumulated squared gradients
    ...
    def train(self):
        """Train using FedAdagrad with server-side Adagrad optimization"""
        print(f"Training with {self.clients_per_round} workers using FedAdagrad")
        print(f"Server LR: {self.server_lr}, Tau: {self.tau}")
        ...
        for i in trange(self.num_rounds, desc=self.desc):
            ...
            # Aggregate using Adagrad-style adaptive optimization
            self.latest_model = self.aggregate_adagrad(csolns)
    ...
    @torch.no_grad()
    def aggregate_adagrad(self, wsolns):
        """
        Aggregate client updates using Adagrad-style adaptive learning rates
        
        Algorithm:
        1. Compute weighted average of client updates (pseudo-gradient)
        2. Accumulate squared pseudo-gradients
        3. Apply adaptive learning rate based on accumulated squares
        """
        total_weight = 0.0
        ...
        for key in current_state.keys():
            ...
            # Adaptive update: x_t+1 = x_t + η * Δ_t / (sqrt(v_t) + τ)
            new_state[key] = current_state[key] + self.server_lr * grad / (torch.sqrt(self.v[key]) + self.tau)
