...
def main():
    ...
    for exp_folder in sorted(os.listdir(results_dir)):
        ...
        # Only process b0_1 (beta=0.1) directories
        if '_b0_1_' not in exp_folder and not exp_folder.endswith('_b0_1'):
            continue
        ...
        # Extract full dataset setting (e.g., "noiid_lbldir_b0_1_k100")
        if dataset:
        ...
        # Extract beta value
        if '_b0_1_' in exp_folder:
            beta = '0.1'
        elif exp_folder.endswith('_b0_1'):
            beta = '0.1'
        
        if dataset and beta == '0.1' and dataset_setting:
...
def generate_latex_table(convergence_data, targets):
    ...
    for dataset in ['fmnist', 'cifar', 'cifar100']:
        ...
        for setting in sorted(settings_data.keys()):
            ...
            # Create LaTeX table
            latex_output.append(f"\n% Table for {dataset.upper()} - {setting} (β = 0.1)")
            latex_output.append("\\begin{table}[H]")
            latex_output.append("\\centering")
            latex_output.append(f"\\caption{{Convergence rounds for {dataset.upper()} ({setting}, β=0.1)}}")
            ...
            latex_output.append("\\label{tab:convergence_" + dataset + "_" + setting.replace("_", "") + "_b01}")
    ...
    # Save to file
    with open('/home/sujit_2021cs35/Github/FedSat/convergence_table_b01.tex', 'w') as f:
        f.write(latex_code)
    print(f"\nLaTeX code saved to: convergence_table_b01.tex")
